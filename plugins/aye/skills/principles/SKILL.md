...
本 skill 同时面向 Claude Code 与 Codex:

- Claude Code:优先使用 `AskUserQuestion` 呈现 2-4 个候选。
- Codex:若当前环境有原生交互式选择工具则使用;否则使用下面的文本 fallback。
- 不管 host 是否有工具,候选都必须保留 **trade-off + 推荐 + 理由**。
