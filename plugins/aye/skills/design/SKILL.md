...
- bug fix / typo / 局部重命名
- 实施方案显然(只有一种合理选法)
- 已有 design.md 的小修(直接更新现有文档,不开新)

---

## 触发场景(用户口语)

满足"必走"清单时,`acceptance` 完成后 conditional 触发,或用户直接喊:
...
> 大决策走 A/B 文档化(本段);**小决策**(2-4 个候选可一句话说清的,如某段实现 idiom / 某个 dep crate / 某个枚举名)优先用交互式选择工具收拍板(Claude Code: `AskUserQuestion`; Codex:可用工具,否则文本 fallback),答完写回本段——见 `principles` 的「AI 不当提问机」段。
